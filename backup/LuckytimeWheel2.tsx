...
import {
  setLuckyTimeResults,
  spinFinished, // if you need from elsewhere
  spinStarted,
...
// art centers (clockwise, top=0°)
const newArray = [
  { name: "Tiger", emoji: "🐯", dig: 0 },
  { name: "Lion", emoji: "🦁", dig: 30 },
  { name: "Leopard", emoji: "🐆", dig: 60 },
  { name: "Pig", emoji: "🐖", dig: 90 },
  { name: "Cow", emoji: "🐄", dig: 120 },
  { name: "Monkey", emoji: "🐒", dig: 150 },
  { name: "Wolf", emoji: "🐺", dig: 180 },
  { name: "Rabbit", emoji: "🐰", dig: 210 },
  { name: "Fox", emoji: "🦊", dig: 240 },
  { name: "Horse", emoji: "🐴", dig: 270 },
  { name: "Bear", emoji: "🐻", dig: 300 },
  { name: "Elephant", emoji: "🐘", dig: 330 },
];
...
// fine-tune if art top-center is slightly off
...
export default function Wheel() {
  ...
  const lastConsumedSpinId = useRef<number>(0); // prevent duplicate runs
  ...
  // compute slot from final rotation
  ...
  useEffect(() => {
    if (!spinId) return;
    if (lastConsumedSpinId.current === spinId) return; // already handled
    if (isSpinning) return; // currently spinning; ignore trigger

    lastConsumedSpinId.current = spinId;
  ...
  return (
    <div className="flex flex-col items-center w-full mx-auto justify-center">
      ...
      {/* Optional debug */}
      {/* <div className="mt-2 text-xs text-gray-400">rot: {rotationDbg}°</div> */}
    </div>
  );
}
